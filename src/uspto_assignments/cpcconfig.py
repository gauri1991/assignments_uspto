...
surface is the PatentSearch/ODP API — ``POST`` to the patent endpoint, query by ``patent_id`` (a
bare grant number, e.g. ``10987654``), requesting the nested ``cpc_current`` fields, authenticated
with an ``X-Api-Key`` header (throttle ~45 requests per minute).
"""
...
# Endpoint/auth defaults — verified 2026-07; re-verify against https://data.uspto.gov live docs.
CPC_ENDPOINT_AS_OF = "2026-07"
DEFAULT_ODP_ENDPOINT = "https://search.patentsview.org/api/v1/patent/"
...
class CpcSourceConfig:
    ...
    # API source: endpoint + the nested CPC field to request; the key comes from ``api_key_env``.
    endpoint: str = DEFAULT_ODP_ENDPOINT
    api_key_env: str = DEFAULT_API_KEY_ENV
    code_field: str = "cpc_current.cpc_group_id"  # nested API field carrying the full CPC symbol
    batch_size: int = 100  # patents per API request (POST body ``patent_id`` list)
    ...
    def from_dict(cls, data: dict[str, Any]) -> CpcSourceConfig:
        ...
        return cls(
            ...
            endpoint=str(data.get("endpoint", DEFAULT_ODP_ENDPOINT)),
            api_key_env=str(data.get("api_key_env", DEFAULT_API_KEY_ENV)),
            code_field=str(data.get("code_field", "cpc_current.cpc_group_id")),
